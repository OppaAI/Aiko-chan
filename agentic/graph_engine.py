        ...
        },
{
        ...
        # ──────────────────────────────────────────────────────────────
        # IT ops playbooks (Jetson-safe: read-only tools, <=6 nodes,
        # max 2 parallel LLM calls so Ministral-3B + 8GB stays responsive)
        # ──────────────────────────────────────────────────────────────
        {
            "id": "it_syscheck",
            "name": "System health check + report (CPU/RAM/disk/processes)",
            "triggers": ["system check", "health check", "syscheck", "how is the system", "cpu usage", "disk usage", "memory usage", "system status"],
            "semantic_triggers": [
                "check if the system is healthy",
                "is the server running out of disk or memory",
                "give me a system health report",
            ],
            "requires_any": ["system", "health", "cpu", "memory", "ram", "disk", "syscheck", "status", "jetson"],
            "capabilities": ["it_ops"],
            "nodes": [
                {"id": "health", "tool": "sys_health", "args": {}},
                {"id": "top", "tool": "process_top", "depends_on": ["health"], "args": {"limit": 8, "sort_by": "cpu"}},
                {"id": "merge", "tool": "combine_evidence", "depends_on": ["health", "top"],
                 "args": {"parts": ["$result:health", "$result:top"]}},
                {"id": "draft", "tool": "synthesize_report", "depends_on": ["merge"],
                 "args": {"evidence": "$result:merge", "prompt": "$prompt", "style": "plain"}},
                {"id": "report", "tool": "write_report", "depends_on": ["draft"],
                 "args": {"title": "$title", "content": "$result:draft", "report_dir": "reports/it"}},
            ],
        },
        {
            "id": "it_log_triage",
            "name": "Log triage: tail + anomaly extract + summary",
            "triggers": ["check logs", "log triage", "what is wrong", "errors in log", "parse logs", "tail log"],
            "semantic_triggers": [
                "check the logs for errors",
                "what went wrong in the recent logs",
                "triage the error log and summarize",
            ],
            "requires_any": ["log", "logs", "error", "triage", "tail", "traceback", "exception"],
            "capabilities": ["it_ops"],
            "nodes": [
                {"id": "triage", "tool": "log_triage", "args": {"log_file": "logs/aiko.log", "lines": 150}},
                {"id": "draft", "tool": "synthesize_report", "depends_on": ["triage"],
                 "args": {"evidence": "$result:triage", "prompt": "Summarize the anomalies and suggest the single most likely next check: $prompt", "style": "plain"}},
                {"id": "report", "tool": "write_report", "depends_on": ["draft"],
                 "args": {"title": "$title", "content": "$result:draft", "report_dir": "reports/it"}},
            ],
        },
        {
            "id": "it_endpoint_check",
            "name": "Endpoint check: DNS + port + HTTP + TLS expiry",
            "triggers": ["is it up", "endpoint check", "check website", "check service", "port check", "ssl expiry", "certificate expiry", "down detector"],
            "semantic_triggers": [
                "check if this website or service is up",
                "is this host reachable and is its certificate ok",
                "probe this endpoint and report status",
            ],
            "requires_any": ["up", "endpoint", "website", "service", "port", "http", "ssl", "certificate", "reachable", "down"],
            "capabilities": ["it_ops"],
            "nodes": [
                {"id": "dns", "tool": "dns_lookup", "args": {"hostname": "$prompt"}},
                {"id": "port", "tool": "port_check", "depends_on": ["dns"], "args": {"host": "$prompt", "port": 443}},
                {"id": "http", "tool": "http_check", "depends_on": ["dns"], "args": {"url": "$prompt"}},
                {"id": "merge", "tool": "combine_evidence", "depends_on": ["dns", "port", "http"],
                 "args": {"parts": ["$result:dns", "$result:port", "$result:http"]}},
                {"id": "draft", "tool": "synthesize_report", "depends_on": ["merge"],
                 "args": {"evidence": "$result:merge", "prompt": "Endpoint status verdict for: $prompt", "style": "plain"}},
                {"id": "report", "tool": "write_report", "depends_on": ["draft"],
                 "args": {"title": "$title", "content": "$result:draft", "report_dir": "reports/it"}},
            ],
        },
        {
            "id": "it_disk_cleanup_preview",
            "name": "Disk cleanup preview (largest files, no deletes)",
            "triggers": ["disk full", "free disk", "clean disk", "large files", "what is using disk"],
            "semantic_triggers": [
                "what is using all the disk space",
                "find large files I can clean up",
                "disk is full, help me preview cleanup",
            ],
            "requires_any": ["disk", "cleanup", "large", "space", "full", "free"],
            "capabilities": ["it_ops"],
            "nodes": [
                {"id": "health", "tool": "sys_health", "args": {}},
                {"id": "large", "tool": "disk_large_files", "depends_on": ["health"],
                 "args": {"directory": "logs", "limit": 10, "max_depth": 3}},
                {"id": "draft", "tool": "synthesize_report", "depends_on": ["large"],
                 "args": {"evidence": "$result:large", "prompt": "Suggest safe cleanup candidates (never system files): $prompt", "style": "plain"}},
                {"id": "report", "tool": "write_report", "depends_on": ["draft"],
                 "args": {"title": "$title", "content": "$result:draft", "report_dir": "reports/it"}},
            ],
        },
        # ── Everyday ──────────────────────────────────────────────────
        {
            "id": "everyday_summarize",
            "name": "Summarize pasted text and save the summary",
            "triggers": ["summarize", "tldr", "tl;dr", "sum up", "condense this"],
            "semantic_triggers": [
                "summarize this text for me",
                "give me the tldr of this",
                "condense this into a few sentences",
            ],
            "requires_any": ["summar", "tldr", "condense", "sum up"],
            "capabilities": ["everyday"],
            "nodes": [
                {"id": "summary", "tool": "text_summarize", "args": {"text": "$prompt", "max_sentences": 5}},
                {"id": "save", "tool": "save_note", "depends_on": ["summary"],
                 "args": {"title": "$title", "content": "$result:summary", "folder": "notes"}},
            ],
        },
        {
            "id": "everyday_translate_save",
            "name": "Translate short text and save both versions",
            "triggers": ["translate", "translation", "in japanese", "in english", "日本語に", "英語に"],
            "semantic_triggers": [
                "translate this to Japanese",
                "translate this to English",
                "what does this mean in another language",
            ],
            "requires_any": ["translat", "日本語", "英語", "japanese", "english"],
            "capabilities": ["everyday"],
            "nodes": [
                {"id": "tr", "tool": "text_translate", "args": {"text": "$prompt", "target": "Japanese"}},
                {"id": "save", "tool": "save_note", "depends_on": ["tr"],
                 "args": {"title": "$title", "content": "$result:tr", "folder": "notes"}},
            ],
        },
        {
            "id": "everyday_morning_brief",
            "name": "Morning brief: system + schedule + note",
            "triggers": ["morning brief", "daily brief", "start of day", "what is today", "day overview"],
            "semantic_triggers": [
                "give me my morning briefing",
                "what does my day look like",
                "brief me on system and schedule",
            ],
            "requires_any": ["brief", "morning", "today", "overview", "schedule"],
            "capabilities": ["everyday", "scheduling", "it_ops"],
            "nodes": [
                {"id": "health", "tool": "sys_health", "args": {}},
                {"id": "sched", "tool": "list_schedule", "depends_on": ["health"], "args": {"include_disabled": False}},
                {"id": "merge", "tool": "combine_evidence", "depends_on": ["health", "sched"],
                 "args": {"parts": ["$result:health", "$result:sched"]}},
                {"id": "draft", "tool": "synthesize_report", "depends_on": ["merge"],
                 "args": {"evidence": "$result:merge", "prompt": "Morning brief for: $prompt", "style": "plain"}},
                {"id": "report", "tool": "write_report", "depends_on": ["draft"],
                 "args": {"title": "$title", "content": "$result:draft", "report_dir": "reports/daily"}},
            ],
        },
        # ── Coding agent (Ministral-3B: small scoped tasks only) ──────
        {
            "id": "code_explain",
            "name": "Explain code: search + read + summarize (read-only)",
            "triggers": ["explain code", "how does", "where is", "what does this do", "walk me through the code"],
            "semantic_triggers": [
                "explain how this part of the codebase works",
                "where is this implemented and what does it do",
                "walk me through this code",
            ],
            "requires_any": ["explain", "how does", "where is", "what does", "code"],
            "capabilities": ["coding", "codebase", "repo"],
            "nodes": [
                {"id": "search", "tool": "codebase_search", "args": {"query": "$prompt", "limit": 5}},
                {"id": "tree", "tool": "repo_file_tree", "depends_on": ["search"], "args": {"prefix": "", "limit": 30}},
                {"id": "merge", "tool": "combine_evidence", "depends_on": ["search", "tree"],
                 "args": {"parts": ["$result:search", "$result:tree"]}},
                {"id": "draft", "tool": "synthesize_report", "depends_on": ["merge"],
                 "args": {"evidence": "$result:merge", "prompt": "Explain clearly for a junior dev: $prompt", "style": "plain"}},
            ],
        },
        {
            "id": "code_small_fix",
            "name": "Small code fix loop: plan -> diff preview -> approval-gated apply -> test",
            "triggers": ["fix bug", "small fix", "patch this", "code fix", "repair this file"],
            "semantic_triggers": [
                "fix this small bug with a minimal patch",
                "make a tiny code change and test it",
                "patch this file safely with review",
            ],
            "requires_any": ["fix", "bug", "patch", "code fix", "repair"],
            "capabilities": ["coding", "repo"],
            "nodes": [
                {"id": "plan", "tool": "code_plan", "args": {"goal": "$prompt"}},
                {"id": "search", "tool": "repo_search_text", "depends_on": ["plan"], "args": {"query": "$prompt", "limit": 10}},
                {"id": "preview", "tool": "code_diff_preview", "depends_on": ["search"],
                 "args": {"relative_path": "", "old_text": "", "new_text": ""}},
                {"id": "apply", "tool": "code_apply_patch", "depends_on": ["preview"], "needs_approval": True,
                 "args": {"relative_path": "", "old_text": "", "new_text": ""}},
                {"id": "lint", "tool": "code_lint", "depends_on": ["apply"], "args": {"relative_path": ""}},
        ...
        # ── Needle2 multi-agent ───────────────────────────────────────
        {
            ...
            "semantic_triggers": [
                ...
                "get multiple agents to look at this",
                "delegate this to the Needle workers",
            ],
            "requires_any": ["multi-agent", "needle", "team", "delegate", "agents"],
            "capabilities": ["multi_agent", "research"],
            "nodes": [
                {"id": "team", "tool": "needle_team_run", "args": {"task": "$prompt"}},
                {"id": "draft", "tool": "synthesize_report", "depends_on": ["team"],
                 "args": {"evidence": "$result:team", "prompt": "Synthesize the team proposals into one answer: $prompt", "style": "professional"}},
                {"id": "report", "tool": "write_report", "depends_on": ["draft"],
                 "args": {"title": "$title", "content": "$result:draft", "report_dir": "reports/multi_agent"}},
            ],
        },
    ]
