...
# ── session-level dynamic anchor (Phase 17) ─────────────────────────────────
# Maintains a per-user ring buffer of recent query embeddings for this process
# and gives a small score boost to memories similar to the current chat's
# mean topic — "what's hot in this chat" — without changing monthly
# consolidation novelty. Lives here alongside the other recall-time scoring
# helpers (spread_activation, memory_max_activation, entity importance) so
# the whole "what to boost at recall" toolkit is in one file.
...
_session_anchor_lock = threading.RLock()
_session_buffers: dict[str, Deque[list[float]]] = defaultdict(
...
def push_query_vector(user_id: str, vector: list[float] | None) -> None:
    ...
    if not MEMORY_SESSION_ANCHOR_ENABLED or not user_id or not vector:
        return
    normed = _normalize_vec(list(vector))
    if normed is None:
        return
    with _session_anchor_lock:
        buf = _session_buffers[user_id]
        # Refresh maxlen if K changed at runtime (rare)
        if buf.maxlen != MEMORY_SESSION_ANCHOR_K:
            _session_buffers[user_id] = deque(buf, maxlen=MEMORY_SESSION_ANCHOR_K)
            buf = _session_buffers[user_id]
        buf.append(normed)
...
def session_mean(user_id: str) -> list[float] | None:
    ...
    if not MEMORY_SESSION_ANCHOR_ENABLED or not user_id:
        return None
    with _session_anchor_lock:
        buf = _session_buffers.get(user_id)
        if not buf:
            return None
        dim = len(buf[0])
        acc = [0.0] * dim
    ...
    return _normalize_vec(acc)


def _anchor_cosine(a: list[float] | None, b: list[float] | None) -> float:
...
def clear_session(user_id: str | None = None) -> None:
    ...
    with _session_anchor_lock:
        if user_id is None:
            ...
            _session_buffers.pop(user_id, None)
    ...
    try:
        ...
        for row in rows:
            ...
            try:
                ...
                normed = _normalize_vec([float(x) for x in vec])
                if normed:
                    out[str(mid)] = normed
            except Exception:
