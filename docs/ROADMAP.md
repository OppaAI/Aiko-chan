...
| Interrupt handling / barge-in — speak over Aiko mid-response | ✅ Done |
...
| Wake word to activate system from Idle Mode | ✅ Done |
...
| Agentic facade | Direct calls | **`agentic/toolkit/tools.py` compatibility facade + graph-first executor + ReAct fallback loop** |
...
- Lane D creates one Threads teaser-list draft per night at 23:00 for tech jobs available today, using only configured CivicJobs.ca and Job Bank Canada RSS feeds filtered by TECH_JOB_KEYWORDS.
