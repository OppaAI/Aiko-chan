"""
Shogi experience store: thin wrapper over the shared core.

Storage: <USER_SPACE_ROOT>/<uid>/agentic/shogi.db (see learn.py).
...
def user_shogi_db_path(uid: str) -> Path:
...
def append_match(uid: str, record: dict[str, Any]) -> None:
    _learn.append_match(uid, GAME, {
        ...
        "you_pts": 1 if record.get("winner") == "you" else 0,
        "aiko_pts": 1 if record.get("winner") == "aiko" else 0,
...
def load_recent(uid: str, limit: int = 20) -> list[dict]:
    ...
    for r in _learn.load_recent(uid, GAME, limit):
        ...
        out.append({
            "ts": r["ts"], "difficulty": r["difficulty"],
...
def build_reflection_prompt(recent: list[dict], st: dict) -> str:
    ...
    for r in recent[-12:]:
        lines.append(
            f"- {r.get('difficulty', '?')}: winner={r.get('winner')} "
            f"end={r.get('end', '?')} moves={r.get('moves_made', 0)}"
        )
    history = "\n".join(lines) if lines else "(no games yet)"
    aiko_rate = (st["aiko_wins"] / st["matches"] * 100) if st["matches"] else 0.0
    return (
        "You coach Aiko, a cat-girl AI that plays Shogi via YaneuraOu (or "
        "casual legal moves when the engine is offline). She learns from "
        "experience WITHOUT retraining: your output tunes her "
        "casual-mistake knob and her table talk.\n\n"
