...
def _score_daily_row(
    ...
    *,
    ...
    # Phase 2: prefer distinct recall days (access_day_count). Fall back to
    # access_count for rows written before the column existed / never touched
    # under Phase 2 (treat raw count as at most 1 "day signal" if day count is 0).
...
def maybe_run_consolidation(memorize, now: datetime | None = None, user_id: str | None = None) -> dict:
    ...
    try:
        ...
        log.warning("Failed to load daily journals for monthly consolidation: %s", exp)
    ...
    try:
        ...
        log.warning("monthly_maintenance failed: %s", exp)
...
def _archive_reports(user_id: str | None = None, keep_days: int = 90) -> dict:
    ...
    for report_file in reports_dir.iterdir():
        ...
        try:
            ...
            log.warning("failed to archive %s: %s", report_file, exp)
            errors += 1
...
def _maintenance_run(user_id: str | None = None, memorize=None) -> dict:
    ...
    try:
        ...
        log.warning("archive_reports failed: %s", exp)
        results["archive_reports"] = {"error": str(exp)}
    ...
    try:
        ...
        log.warning("prune_knowledge failed: %s", exp)
        results["prune_knowledge"] = {"error": str(exp)}
    ...
    try:
        ...
        log.warning("vacuum_knowledge failed: %s", exp)
        results["vacuum_knowledge"] = {"error": str(exp)}
    ...
    try:
        ...
        log.warning("vacuum_memory failed: %s", exp)
        results["vacuum_memory"] = {"error": str(exp)}
