...
let gestureTarget = {};
let lastVisemeAt = 0;
const GESTURES = ['lookAround', 'lookAtHand', 'hairBrush', 'fingerPlay', 'meetGaze', 'curiousTilt', 'shiftWeight', 'stretchNeck'];
...
function pickGesture() {
  ...
  gestureT = 0;
  gestureDuration = GESTURE_DURATION[gesture] ?? 2.6;
  gestureTarget = {
    side,
    look: side * (0.22 + Math.random() * 0.18),
    tilt: side * (0.08 + Math.random() * 0.07),
  };
}
...
function applyFingerCurl(side, intensity) {
  ...
  for (const name of FINGER_BONES) {
    ...
    if (bone) bone.rotation.z += intensity * (0.08 + pulse * 0.10);
  }
}
...
function applyGestures(dt) {
  ...
  switch (gestureState) {
    case 'lookAround':
      if (head) { head.rotation.y += gestureTarget.look * held; head.rotation.x += Math.sin(eased * Math.PI) * 0.025; }
      if (neck) neck.rotation.y += gestureTarget.look * 0.22 * held;
      break;
    case 'lookAtHand':
      if (side < 0) {
        if (lUA) { lUA.rotation.x = REST.leftUpperArm.x - intensity * 0.28; lUA.rotation.z = REST.leftUpperArm.z - intensity * 0.42; }
        if (lLA) { lLA.rotation.x = REST.leftLowerArm.x - intensity * 0.58; lLA.rotation.z = REST.leftLowerArm.z - intensity * 0.18; }
        if (lH) { lH.rotation.x = REST.leftHand.x + intensity * 0.16; lH.rotation.y = REST.leftHand.y + intensity * 0.26; }
      } else {
        if (rUA) { rUA.rotation.x = REST.rightUpperArm.x - intensity * 0.28; rUA.rotation.z = REST.rightUpperArm.z + intensity * 0.42; }
        if (rLA) { rLA.rotation.x = REST.rightLowerArm.x - intensity * 0.58; rLA.rotation.z = REST.rightLowerArm.z + intensity * 0.18; }
        if (rH) { rH.rotation.x = REST.rightHand.x + intensity * 0.16; rH.rotation.y = REST.rightHand.y - intensity * 0.26; }
      }
      if (head) { head.rotation.y += side * intensity * 0.18; head.rotation.x += intensity * 0.08; }
      break;
    case 'hairBrush':
      if (side < 0) {
        if (lUA) { lUA.rotation.z = REST.leftUpperArm.z - intensity * 0.62; lUA.rotation.x = REST.leftUpperArm.x - intensity * 0.32; }
        if (lLA) { lLA.rotation.x = REST.leftLowerArm.x - intensity * 0.68; lLA.rotation.z = REST.leftLowerArm.z - intensity * 0.26; }
        if (lH) { lH.rotation.y = REST.leftHand.y + intensity * 0.28; lH.rotation.z = REST.leftHand.z - intensity * 0.16; }
      } else {
        if (rUA) { rUA.rotation.z = REST.rightUpperArm.z + intensity * 0.62; rUA.rotation.x = REST.rightUpperArm.x - intensity * 0.32; }
        if (rLA) { rLA.rotation.x = REST.rightLowerArm.x - intensity * 0.68; rLA.rotation.z = REST.rightLowerArm.z + intensity * 0.26; }
        if (rH) { rH.rotation.y = REST.rightHand.y - intensity * 0.28; rH.rotation.z = REST.rightHand.z + intensity * 0.16; }
      }
      if (head) { head.rotation.z -= side * intensity * 0.04; head.rotation.y += side * intensity * 0.03; }
      break;
    case 'fingerPlay':
      if (head) { head.rotation.x += intensity * 0.06; head.rotation.y += side * intensity * 0.10; }
      if (side < 0) {
        if (lLA) lLA.rotation.x = REST.leftLowerArm.x - intensity * 0.26;
        if (lH) lH.rotation.y = REST.leftHand.y + intensity * 0.18;
      } else {
        if (rLA) rLA.rotation.x = REST.rightLowerArm.x - intensity * 0.26;
        if (rH) rH.rotation.y = REST.rightHand.y - intensity * 0.18;
      }
    ...
    case 'meetGaze':
      ...
      break;
    case 'curiousTilt':
      if (head) { head.rotation.z += gestureTarget.tilt * held; head.rotation.x -= 0.018 * intensity; }
      if (neck) neck.rotation.z += gestureTarget.tilt * 0.45 * held;
      break;
    case 'shiftWeight':
